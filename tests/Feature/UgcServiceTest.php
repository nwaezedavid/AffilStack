...
/**
 * UgcService::run() (shared by angles() and content()) runs inside a queue
 * worker (RunUgcGeneration, $tries = 1, no failed() handler) and is
 ...
 * every generation module in this audit. Exercised here through angles() —
 * content() shares the exact same run() method, so it shares the same fix.
 */
class UgcServiceTest extends TestCase
{
    ...
    public function test_angles_spends_credits_and_completes_on_success(): void
    {
        ...
        $this->mock(AIProvider::class, function (MockInterface $mock) {
            $mock->shouldReceive('generateJson')->once()->andReturn(['angles' => []]);
        });

        app(UgcService::class)->angles($generation);
    ...
    public function test_angles_fails_cleanly_instead_of_crashing_on_an_unexpected_provider_error(): void
    {
        $user = User::factory()->create(['credits_balance' => 100]);
        $offer = $this->offer($user);
        $generation = $this->queuedGeneration($offer, $user);
