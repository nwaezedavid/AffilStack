...
/**
 * CompetitorAngleService::run() (shared by scan()) runs inside a queue
 * worker (RunCompetitorAngleGeneration, $tries = 1, no failed() handler)
 * and is documented to "always leave the generation in a terminal state and
 * never throw". See BlogArticleServiceTest for the same bug class found
 * across every generation module in this audit.
 */
class CompetitorAngleServiceTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_scan_spends_credits_and_completes_on_success(): void
    {
        ...
        $this->mock(AIProvider::class, function (MockInterface $mock) {
            $mock->shouldReceive('generateJson')->once()->andReturn(['common_angles' => [], 'differentiation_opportunity' => 'x', 'recommended_adjustment' => 'y']);
        });

        app(CompetitorAngleService::class)->scan($generation);

        $generation->refresh();
        $this->assertSame('completed', $generation->status);
        $this->assertSame(6, $generation->credits_spent);
        $this->assertSame(100 - 6, $user->fresh()->credits_balance);
    }

    public function test_scan_fails_cleanly_instead_of_crashing_on_an_unexpected_provider_error(): void
    {
        $user = User::factory()->create(['credits_balance' => 100]);
        $offer = $this->offer($user);
        $generation = $this->queuedGeneration($offer, $user);
