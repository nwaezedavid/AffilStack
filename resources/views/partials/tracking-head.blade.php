{{--
    Sitewide analytics/ad-pixel <head> scripts, shared by the marketing and
    dashboard layouts so tracking covers logged-out visitors and logged-in
    customer behavior alike. Expects $gtmPublicId, $gaId, $metaPixelId,
    $tiktokPixelId to already be in scope (set by the including layout).
--}}
@if ($gtmPublicId)
    <script>
        ...
        })(window,document,'script','dataLayer','{{ $gtmPublicId }}');
    </script>
@elseif ($gaId)
...
@if ($metaPixelId)
    <script>
        ...
        fbq('init', {{ json_encode((string) $metaPixelId) }});
        fbq('track', 'PageView');
    </script>
...
@if ($tiktokPixelId)
    <script>
        !function (w, d, t) {
          ...
          ttq.load({{ json_encode((string) $tiktokPixelId) }});
          ttq.page();
        }(window, document, 'ttq');
    </script>
@endif
